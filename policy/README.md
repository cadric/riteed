...
- `fuzz_targets.targets_required` is the single required-target list for source files, exact Cargo bin registrations, seed corpora, registry coverage, and the scheduled/manual CI fuzz loop. Target-specific seed widths, scan caps, and required semantic classes remain policy-owned values.
