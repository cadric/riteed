...
- **Korrektion (2026-07-06, remediation; empirisk verificeret på git 2.54.0 — samme version som bundlet):** Unstage-delpåstanden ovenfor er **afkræftet**. `git ls-tree HEAD -- '[id].txt'` matcher det literale filnavn og wildmatcher ikke (`'i.*'` og `'[ix].txt'` returnerer begge tom output), så `unstage_path` når aldrig tom-output→`update-index --force-remove`-grenen; unstage opfører sig korrekt allerede uden fixet. Discard-delpåstanden er **bekræftet**: `git restore --worktree -- '[id].txt'` uden env-varen reverterer både `[id].txt` og glob-matchede søskende (`i.txt`); med `GIT_LITERAL_PATHSPECS=1` kun den navngivne fil. Fix og Critical-severity uændret (reelt søskende-datatab via Discard). Remediation-planen (`docs/superpowers/plans/2026-07-06-audit-p0-p1-remediation.md`, Task 1) bruger restore-testen som bindende regressionstest og beholder unstage-testen som characterization-vagt.
