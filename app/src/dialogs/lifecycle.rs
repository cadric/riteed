...
static COMPARE_STATES: AtomicUsize = AtomicUsize::new(0);
...
pub(crate) fn reset_dialog_leak_counters_for_tests() {
    COMPARE_STATES.store(0, Ordering::SeqCst);
...
pub(crate) fn assert_dialog_leak_counters_clear_for_tests() {
    assert_eq!(COMPARE_STATES.load(Ordering::SeqCst), 0);
...
pub(crate) fn dialog_leak_counters_clear_for_tests() -> bool {
    COMPARE_STATES.load(Ordering::SeqCst) == 0
...
fn counter(kind: DialogLeakKind) -> &'static AtomicUsize {
    match kind {
        DialogLeakKind::Compare => &COMPARE_STATES,
