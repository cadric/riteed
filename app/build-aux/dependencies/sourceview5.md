...
- Local patch note: Riteed v5a adds a small vendored safe wrapper in `src/file_loader.rs` for `set_candidate_encodings(...)` so the app can implement deterministic “Reopen With Encoding…” without introducing `unsafe` in `app/src`.
